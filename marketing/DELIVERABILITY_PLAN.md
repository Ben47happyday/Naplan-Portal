...
| Known limitation: Sent Items self-opens | ⚠️ Accepted 2026-09-14, not fixed | Graph auto-saves a copy of every sent message to Sent Items with the live tracking pixel/click link still embedded. Re-rendering that copy later — anyone opening it in the mailbox, or Outlook/Exchange's own preview or indexing — refires the same open/click event, indistinguishable from a genuine recipient action (confirmed live 2026-09-12; this is what corrupted an earlier engagement report). Two direct fixes were tried and both are dead ends: deleting the Sent Items copy after send is blocked by Claude Code's own audit-tampering guardrail, and `PATCH`-ing its body to a de-tracked version is rejected live by Exchange itself (`403 ErrorAccessDenied` on `BodyHtmlBinary` — Microsoft won't let a sent item's body be rewritten via Graph). The alternative that *would* fully prevent it — sending via the `sendMail` action with `saveToSentItems: false` instead of the current raw-MIME flow — was rejected because it would give up the real `multipart/alternative` body and the `List-Unsubscribe` header (both above, ✅ live). **Accepted workaround:** never open the Sent Items copy in the mailbox to check what went out — use `python send_campaign.py --config <config> --preview-out <file>` instead, which renders the exact content with no network calls. When reviewing opens/clicks, treat a hit that lines up with a known internal check as a likely false positive rather than genuine engagement (see the campaign-report skill's bot-detection heuristics for the general version of this). |
